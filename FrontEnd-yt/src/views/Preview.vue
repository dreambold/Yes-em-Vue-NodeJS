    ...
    <v-layout ref="topBar" column>
      ...
      <div style="display: flex; flex-direction: column; margin-top: 0px">
        ...
        <v-flex ref="principalDiv" xs12 :style="isMobile ? 'margin-bottom: 3px;' : ''" v-if="getCompanyData === null ? false : (getCompanyData.published ? true : (currentUser.identifier === $route.params.identifier ? true : false))">
            ...
            <v-layout v-if="!$vuetify.breakpoint.smAndDown && !showService && type == 0 && !showClients" justify-center align-center column :class="$vuetify.breakpoint.smAndDown ? 'px-3 pb-3' : 'px-4'" style="margin-bottom: 20px">
              <!-- <HeaderBox 
                :allStad="getAllStad()"
                :allValidations="getAllValidations()"
                :getAllClients="getAllClients()"
              ...
              <div style="width: 1186px;text-align: left;margin-bottom: 151px;margin-top: 8px; display: flex; flex-direction: column">
                                      ...
                                      <div style="display: flex; align-items: center; margin-top: 12px; margin-left: 10px;">
                                          ...
                                          <div style="color: #333333;text-align: left;width: 219px;margin-left: 6px;">
                                              <span style="font-family: 'Roboto-regular'; font-size: 13px; color: #0654ba;">
                                                {{getAllClients()}} clientes han validado, una o varias, opciones de las estadisticas
                                              </span>
                                          ...
                                          <div class="boxScale" v-tooltip.bottom="`La metodología seguida para la recolección de datos ha sido la encuesta. Un total de ${getAllClients()} clientes han dado su opinión validando opciones de respuesta o escribiendo la suya propia.`">
                                            <div style="display: flex; align-items: flex-start;flex-direction: column; margin-left: 30px; margin-right: 30px; width: 134px">
                                                <div style="display: flex; flex-direction: row; align-items: center;">
                                                    ...
                                                    <span style="font-family: 'Roboto-medium'; font-size: 14px; color: #777777">{{getAllClients()}}</span>
                                                </div>
              ...
              <div style="width: 946px;display: flex;align-items: flex-start;margin-top: 10px;">
        ...
        <v-flex xs12 v-if="!$vuetify.breakpoint.smAndDown">
          ...
          <v-layout justify-center align-center column :class="$vuetify.breakpoint.smAndDown ? 'px-3 pb-3' : ''">
            <div :style="`width: ${showService ? '100%' : '1222px'}; height: 100%;`">
                ...
                <div style="display: flex;width: 100%;align-items: flex-start;justify-content: space-between;text-align: left;">
                  ...
                  <div v-if="isLoad && all" style="display: flex;width: 100%;align-items: flex-start;justify-content: space-between;text-align: left;">
                    <div>
                        ...
                        <li @click="solutionSelectedId = 'home'" :style="`margin-bottom: 14px; width: 202px;display: flex;height: 32px;border-radius: 3px;cursor: pointer;align-items: center;padding: 0 10px 0 10px;background: ${solutionSelectedId == 'home' ? '#e6e6e6' : 'transparent'}`">
                            <span style="font-size: 14px; color: #333333">Inicio</span>
                        </li>
                        <li 
                            v-for="(solution, index) in getCompanyData.solutions.filter(({ active }) => active)" 
                            :key="index"
                            @click="solutionSelectedId = solution._id"
                            :style="`margin-bottom: 14px; width: 202px;display: flex;height: 100%;border-radius: 3px;cursor: pointer;align-items: center;padding: 4px 10px 4px 10px;background: ${solutionSelectedId == solution._id ? '#e6e6e6' : 'transparent'}`"
                        >
                            <span :style="`font-size: 14px; color: ${solutionSelectedId == solution._id ? '#666666' : '#777777'}`">{{transformString(solution.category)}}</span>
                        </li>   
                        </ul>
                    </div>

                    <div v-if="solutionSelectedId == 'home'" style="width: 605px; height: 100%; margin-bottom: 40px">
                      <PreviewBoxOld
                        v-for="(solution, j) in getCompanyData.solutions" 
                        :key="j"
                        :all="true"
                        :solution="solution"
                      ...
                      <!-- <div v-for="(solution, j) in getCompanyData.solutions" :key="j" style="width: 605px;height: 100%;margin-bottom: 40px;background: white;border: 1px solid #cecece;border-radius: 4px;">
                        <div style="width: 100%; height: 100%; border: 1px solide #cecece">
                          <div style="width:100%; height: 50px; padding: 0 15px 0 30px; display: flex; align-items: center; border-bottom: 1px solid #ececec">
                            <span style="font-family: 'Roboto-medium'; font-size: 16px; color: #333333">{{transformString(solution.category)}}</span>
                          </div>
                          <div style="text-align: left;width: 100%;height: 100%;padding: 15px 68px 15px 30px;border-bottom: 1px solid #ececec;">
                            <span style="font-family: 'Roboto-regular'; font-size: 16px; color: #333333">{{solution.description}}</span>
                          </div>
                          <div v-for="(item, j) in solution.surveys" :key="j" style="width: 100%;height: 155px;padding: 0 30px 0 30px;display: flex;align-items: center;">
                            <div>
                              <img @click="_solution = solution; scrollTop(); selectSurvey(item);" src="/static/images/cajaservicio1.png" alt="stadistic" style="cursor: pointer; margin-right: 2px;"/>
                            </div>
                            <div style="width: 460px; height: 100%; display: flex;flex-direction: column;text-align: left;margin-left: 20px;justify-content: space-evenly;border-bottom: 1px solid #ececec;">
                              <div>
                                <span @click="_solution = solution; scrollTop(); selectSurvey(item);" style="color: #898fa1; font-family: 'Roboto-regular'; font-size: 13px; cursor: pointer;">{{transformString(item.description)}}</span>
                              </div>
                              <div>
                                <span @click="_solution = solution; scrollTop(); selectSurvey(item);" style="width: 395px; height: 52px; font-family: 'Roboto-bold'; cursor: pointer; color: #333333;font-size: 18px;">
                                  {{item.question}}
                                </span>
                              </div>
                              <div style="display: flex; align-items: center">
                                <div @click=" _solution = solution; changeRoute(solution.category, 'clientes-encuestados'); scrollTop(); showClients = true; isLoad = true; all = false;" style="display: flex; align-items: center">
                                  <img src="/static/images/cajaservicio2.png" style="margin-right: 10px;cursor: pointer; width: 22px" alt="icon header clients">
                                  <span style="font-family: 'Roboto-medium'; font-size: 14px; color: #333333">{{getHappyClientsBySurvey(item).length}}</span>
                                </div>
                                <div @click="_solution = solution; scrollTop(); selectSurvey(item);" style="display: flex; align-items: center; margin-left: 30px;cursor: pointer;">
                                  <img src="/static/images/cajaservicio3.png" style="margin-right: 0px" alt="icon header clients">
                                  <span style="font-family: 'Roboto-medium'; font-size: 14px; color: #333333">{{getValidationsBySurvey(item)}}</span>
                                </div>
                              </div>
                            </div>
                          </div>
                          <div style="width: 100%;height: 155px;padding: 0 30px 0 30px;display: flex;align-items: center;">
                            <div>
                              <img @click="_solution = solution; changeRoute(solution.category, 'clientes-encuestados'); scrollTop(); showClients = true; isLoad = true; all = false;" src="/static/images/cajaservicio2.png" alt="stadistic" style="cursor: pointer; margin-right: 2px;"/>
                            </div>
                            <div style="width: 460px; display: flex;flex-direction: column;text-align: left;margin-left: 20px;justify-content: space-evenly;">
                              <div>
                                <span @click="_solution = solution; changeRoute(solution.category, 'clientes-encuestados'); scrollTop(); showClients = true; isLoad = true; all = false;" style="width: 395px; height: 52px; font-family: 'Roboto-bold'; cursor: pointer; color: #333333;font-size: 18px;">
                                  Clientes Encuestados
                                </span>
                              </div>
                              <div style="display: flex; align-items: center; margin-top: 15px;">
                                <div @click="_solution = solution; changeRoute(solution.category, 'clientes-encuestados'); scrollTop(); showClients = true; isLoad = true; all = false;" style="cursor: pointer; display: flex; align-items: center">
                                  <img src="/static/images/cajaservicio2.png" style="margin-right: 10px;cursor: pointer; width: 22px" alt="icon header clients">
                          ...
                          <div style="width: 100%; display: flex; justify-content: center; align-items: center; height: 54px; border-top: 1px solid #ececec">
                            <span style="color: gray; font-size: 14px; cursor: pointer;">
                    ...
                    <div v-if="solutionSelectedId !== 'home'" style="width: 605px; height: 100%; margin-bottom: 40px">
                        <PreviewBoxOld
                          v-for="(solution, j) in getCompanyData.solutions.filter(({ _id }) => solutionSelectedId == _id)" 
                          :key="j"
                          :all="false"
                          :solution="solution"
                          @distpachNavigateToSurvey="_solution = solution; scrollTop(); selectSurvey($event);"
                      ...
                      <!-- <div v-for="(solution, j) in getCompanyData.solutions.filter(({ _id }) => solutionSelectedId == _id)" :key="j" style="width: 605px;height: 100%;background: white;margin-bottom: 40px;border: 1px solid #cecece;border-radius: 4px;">
                        <div style="width: 100%; height: 100%; border: 1px solide #cecece">
                          <div style="width:100%; height: 50px; padding: 0 15px 0 30px; display: flex; align-items: center; border-bottom: 1px solid #ececec">
                            <span style="font-family: 'Roboto-medium'; font-size: 16px; color: #333333">{{transformString(solution.category)}}</span>
                          </div>
                          ...
                          <div v-for="(item, j) in solution.surveys" :key="j" style="width: 100%;height: 155px;padding: 0 30px 0 30px;display: flex;align-items: center;">
                            <div>
                              <img @click="_solution = solution; scrollTop(); selectSurvey(item);" src="/static/images/cajaservicio1.png" alt="stadistic" style="margin-right: 2px;cursor: pointer;"/>
                            </div>
                            <div style="width: 460px; height: 100%; display: flex;flex-direction: column;text-align: left;margin-left: 20px;justify-content: space-evenly;border-bottom: 1px solid #ececec;">
                              <div>
                                <span @click="_solution = solution; scrollTop(); selectSurvey(item);" style="color: #898fa1; font-family: 'Roboto-regular'; font-size: 13px; cursor: pointer;">{{transformString(item.description)}}</span>
                              ...
                              <div>
                                <span @click="_solution = solution; scrollTop(); selectSurvey(item);" style="width: 395px; height: 52px; font-family: 'Roboto-bold'; font-size: 18px;color: #333333;cursor: pointer;">
                                  {{item.question}}
                                </span>
                              </div>
                              <div style="display: flex; align-items: center">
                                ...
                                <div @click="_solution = solution; scrollTop(); selectSurvey(item);" style="display: flex; align-items: center; margin-left: 30px;">
                                  <img src="/static/images/cajaservicio3.png" style="margin-right: 0px;cursor: pointer;" alt="icon header clients">
                                  <span style="font-family: 'Roboto-medium'; font-size: 14px; color: #333333">{{getValidationsBySurvey(item)}}</span>
                                </div>
                              </div>
                            </div>
                          ...
                          <div style="width: 100%;height: 155px;padding: 0 30px 0 30px;display: flex;align-items: center;">
                            ...
                            <div style="width: 460px; display: flex;flex-direction: column;text-align: left;margin-left: 20px;justify-content: space-evenly;">
                              ...
                              <div style="width: 100%; display: flex; justify-content: center; align-items: center; height: 54px; border-top: 1px solid #ececec">
                                <span style="color: gray; font-size: 16px; cursor: pointer;">
                                  Ver todas las estadísticas <v-icon  style="color: #696969; font-size: 16px; margin-left: 5px">keyboard_arrow_down</v-icon>
                                </span>
                              </div> 
                            </div>
                    ...
                    <div style="margin-left: 0px;">
                      ...
                      <div style="margin-top: 0px; text-align: left">
                        <span style="color: #333333; font-size: 22px; font-family: 'Roboto-light';">Datos clave</span>
                      </div>

                      <div style="border: 0.7px solid #cecece; width: 90px; margin: 15px 0 20px 0"></div>
                      
                      <div style="width: 310px; margin-bottom: 15px">
                        <span style="color: #636363; font-family: 'Roboto-regular'; font-size: 13px">
                          Los datos claves ofrecen un resumen sobre las estadisticas de '{{getCompanyData.name}}' y enlanzan con la pagina de la estadistica correspondiente
                        </span>
                      </div>

                      <div v-for="(solution, index) in getCompanyData.solutions.filter(({ active }) => active)" :key="index" style="margin-bottom: 30px">
                        <div style="margin-bottom: 10px; text-align: left"><span style="color: #333333; font-family: 'Roboto-bold'; font-size: 14px; text-transform: uppercase">{{transformString(solution.category)}}</span></div>
                        <div style="height: 100%;border-radius: 2px;width: 100%;border-top: 1px solid #cecece;border-right: 1px solid #cecece;border-left: 1px solid #cecece;">
                          <!-- .filter(({active}) => active).sort((a, b) => a.validations.length - b.validations.length) -->
                          <div v-for="(option, j) in allOptions(solution.surveys)" :key="j" @click="_solution = solution; scrollTop(); selectSurvey(solution.surveys.filter(({ _id }) => _id == option.surveyId)[0]);">
                            <div :style="`cursor: pointer;width: 320px;display: flex;height: 110px;padding: 0px 23px 0px;border-bottom: 1px solid rgb(206, 206, 206);background: white;text-align: left;align-items: flex-start;flex-direction: column;justify-content: center;`">
                              <div><span :style="`font-size: 15px; color: #333333; font-family: 'Roboto-regular'`">{{option.optionDescription}}</span></div>
                              <div style="margin-top: 4px"><span style="font-size: 14px; color: #2b6dad; font-family: 'Roboto-light'">{{option.validations.length}} Validaciones</span></div>
                            </div>
                          </div>
                        </div>
                        <div v-if="solution.surveys.filter(({ active }) => active).length == 0" style="margin-top: 15px">
...
import PreviewBoxOld from '../components/PreviewBoxOld'
...
export default {
    name: 'home',
    components: {
        ContactComponent, Tabs, SurveyContact, MobileSurveyCard,SurveyContactMobile, PreviewBoxOld, PreviewOptionsBox,
    ...
    watch: {
      '$route.params': function (id) {
        ...
        if (this.$route.params.solution && this.$route.params.stad && this.$route.path.includes('clientes-encuestados')){
              ...
              this.totalValidations = this.getTotalValidations(this._solution);
              //console.log('SHOW OPTION VIEW', (this._solution.surveys.filter((survey) => this.removeAccents(survey.description).toLowerCase().trim().replace(/\s/g, '') == this.removeAccents(this.$route.params.stad).toLowerCase().trim().replace(/-/g, '')))[0])
              this.scrollTop(); 
    ...
    methods: {
      ...
      clickToGoToClients(e) {
        ...
        //console.log('clientsBySolution', this.clientsBySolution)
      },
      selectSurvey(survey) {
      ...
      filterClickAction(identifier, category, menu) {
        ...
        this._solution = selectedService[0];
        //console.log('SELECTED SERVICE', this._solution)
        this.totalValidations = this.getTotalValidations(this._solution);
      ...
      clickToActiveItem(identifier, category) {
        // //console.log('IIIDDENTIFIER', identifier)
        // //console.log('CATTTEGORY', category)
      ...
      dispatchContactBox(event) {
        console.log('dispatchContactBox', event)
      ...
      validate() {
        ...
        //console.log(`LOCATION: http://yesthem.com/${this.getCompanyData.identifier}/${this.removeAccents(this._solution.identifier).toLowerCase()}/validate`)
      ...
      showAllClients() {
        //console.log('Clients to SHOW', this.clientsToShow)
    ...
    beforeUpdate() {
      if(this.getCompanyData) {
        ...
        if(this.byUrl === 1 || this.byUrl === 2 || this.byUrl == 3) {
          ...
          //// console.log(this.$route.params.solution, this.$route.params.stad && !this.$route.path.includes('clientes-encuestados'))
