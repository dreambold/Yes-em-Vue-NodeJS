      ...
      <div style="display: flex; flex-direction: column; margin-top: 0px">
        ...
        <v-flex ref="principalDiv" xs12 :style="isMobile ? 'margin-bottom: 3px;' : ''" v-if="getCompanyData === null ? false : (getCompanyData.published ? true : (currentUser.identifier === $route.params.identifier ? true : false))">
            ...
            <v-layout v-if="!$vuetify.breakpoint.smAndDown && !showService && type == 0 && !showClients" justify-center align-center column :class="$vuetify.breakpoint.smAndDown ? 'px-3 pb-3' : 'px-4'" style="margin-bottom: 0px">
              ...
              <div style="width: 1258px;text-align: left;margin-top: 8px; display: flex; flex-direction: column">
                  <span style="font-size: 14px; font-family: 'Roboto-regular'; color: #666666">{{getCompanyData.name}} > Estadisticas Validadas por Clientes</span>
                  <!-- <div :style="`width: 1258px; height: 275px; margin-top: 10px; display: flex; justify-content: center; align-items: flex-start; background-image: url(/static/images/uploads/${getCompanyData.coverImage}?${getCompanyData.coverImage.toString(36).substr(2, 9)}; background-size: cover; background-position: center; background-repeat: no-repeat; padding: 0;`"> -->
                      <div style="width: 1258px; height: 380px; background: transparent; margin-top: 10px; display: flex; flex-direction: column; border-top: 1px solid #cecece; border-bottom: 1px solid #cecece; border-left: 1px solid #cecece; border-right: 1px solid #cecece">
                          <div :style="`width: 100%;height: 100%; background-image: url(/static/images/uploads/${getCompanyData.coverImage}?${getCompanyData.coverImage.toString(36).substr(2, 9)}; background-size: cover; padding: 17px 27px 30px;display: flex;flex-direction: row;align-items: flex-start;`">
                          </div>
                          <div style="width: 100%;height: 186px;display: flex; align-items: center;justify-content:space-between;background: white;padding: 10px 27px 30px 27px;border-bottom-left-radius: 5px;border-bottom-right-radius: 5px;">
                                <div style="display: flex; flex-direction: column;">
                                    <div style="width: 425px; display: flex; align-items: center; align-items: flex-end; margin-top: -66px">  
                                        <div>
                                            <div v-if="getCompanyData.logo" :style="`border: 1px solid #cecece; background-image: url(/static/images/uploads/${getCompanyData.logo}##${Math.random().toString(36).substring(7)}); margin-top: 0px; background-size: cover; border-radius: 0px; background-position: center; background-repeat: no-repeat; width: 148px !important; height: 148px !important;`"></div>
                                        </div>
                                        <div style="display: flex; flex-direction: column; margin-left: 20px;">
                                            <div>
                                                <span style="font-size: 26px; font-family: 'Roboto-bold'; color: #111111">{{getCompanyData.name}}</span>
                                            </div>
                                            <div>
                                                <span style="color: #666666; font-size: 16px;">Estadisticas validadas por clientes</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div style="width: 490px; margin: 12px 0 6px 0;">
                                        <span style="color: #111111; font-size: 15px;">Estadisticas validadas por clientes sobre la actividad de {{getCompanyData.description}} desarrollada por {{getCompanyData.name}}</span>
                                    </div>
                                    <div>
                                      <button type="button" style="width: 200px; height: 32px; font-family: 'Roboto-light'; display: flex; align-items: center; justify-content: center; outline: none; background: white; border-radius: 3px; color: #016bf2; font-size: 15px; border: 1px solid rgb(2, 107, 242);">
                                        Contacta en 1 paso
                                      </button>
                                    </div>
                                </div>
                                <div style="display: flex;align-items: flex-end;justify-content: space-between;flex-direction: column;">
                                    <div style="cursor: pointer; margin-bottom: 30px">
                                      <span style="font-size: 12px; font-family: 'Roboto-regular'; color: #111111; display: flex; align-items: center">
                                        <span style="font-family: 'Roboto-regular'; margin-right: 4px;">{{getCompanyData.industry}}</span>  | <img src="/static/images/header/pin.png" style="margin: 0 3px 0 2px" alt="Pin Country" /> {{getCompanyData.country}}
                                      </span>
                                    </div>
                                    <div style="display: flex; align-items: center; margin-top: 10px">
                                        <div class="boxScale" v-tooltip.bottom="'Número total de estadísticas publicadas. Cada estadística se ha ocupado de la recolección, agrupación, presentación y análisis de datos. Estos datos tratan sobre la experiencia de clientes con los productos o servicios que contrato.'">
                                          <div style="display: flex; cursor: pointer; align-items: flex-start; flex-direction: column">
                                              <div style="display: flex; flex-direction: row; align-items: center">
                                                  <img src="/static/images/header/headerestad.png" style="margin-right: 8px" alt="icon header stadistics">
                                                  <span style="font-family: 'Roboto-regular'; font-size: 14px; color: #111111">{{getAllStad()}}</span>
                                              </div>
                                              <span style="font-family: 'Roboto-regular'; font-size: 13px; color: #111111">Estadísticas</span>
                                          </div>
                                        </div>


                                        <div class="boxScale" v-tooltip.bottom="`La metodología seguida para la recolección de datos ha sido la encuesta. Un total de clientes ha dado su opinión validando opciones de respuesta o escribiendo la suya propia.`">
                                          <div style="display: flex; cursor: pointer; align-items: flex-start;flex-direction: column; margin-left: 40px; margin-right: 30px; width: 134px">
                                              <div style="display: flex; flex-direction: row; align-items: center;">
                                                  <img src="/static/images/header/headerclient.png" style="margin-right: 8px" alt="icon header clients">
                                                  <span style="font-family: 'Roboto-regular'; font-size: 14px; color: #111111">{{getAllClients().length}}</span>
                                              </div>
                                              <span style="font-family: 'Roboto-regular'; font-size: 13px; color: #111111">Clientes encuestados</span>
                                          </div>                    
                                        </div>
                                        ...
                                        <div class="boxScale" v-tooltip.bottom="'Se ha solicitado a los clientes dar su opinión en forma de validación. Se busca así demostrar que las estadísticas son válidas para el propósito que buscan. Las validaciones se reparten de una manera no proporcional en todas las estadísticas publicadas.'" >
                                          <div style="display: flex; cursor: pointer; align-items: flex-start; flex-direction: column">
                                              <div style="display: flex; flex-direction: row; align-items: center;">
                                                  ...
                                                  <span style="font-family: 'Roboto-regular'; font-size: 14px; color: #111111">{{getAllValidations()}}</span>
                                              </div>
                                              <span style="font-family: 'Roboto-regular'; font-size: 13px; color: #111111">Validaciones de clientes</span>
                                          </div>                    
                                        </div>
                                    </div>
                                    <div style="display: flex; align-items: center; margin-top: 18px;">
                                      <div  align="left" class="" style="margin-left: -8px" v-for="(client, index) in getAllClients().slice(0,3)" v-bind:key="index">
                                          <div :style="`padding-top: 7px; background-size: cover; border: 1px solid #e3e3e3;border-radius: 50%; width: 29px; height: 29px; background-image: url(${Number.isInteger(parseInt(client.pictureUrl.split('.')[0])) ? '/static/images/faces/' : client.pictureUrl.includes('http') ? '' : '/'}${client.pictureUrl})`">
                                          </div>
                                      </div>
                                      <span style="color: #016bf2;font-family: 'Roboto-regular'; margin-left: 12px;font-size: 14px">{{getAllClients().length-3}} clientes han validado opciones de las estadísticas</span>
                                    </div>
                                </div>
                          </div>
                      </div>
        ...
        <v-flex xs12 v-if="!$vuetify.breakpoint.smAndDown">
          ...
          <v-layout justify-center align-center column :class="$vuetify.breakpoint.smAndDown ? 'px-3 pb-3' : ''">
            <div :style="`width: ${showService ? '100%' : '1258px'}; height: 100%; background: #fafafa; border-bottom: 1px solid #cecece; border-right: 1px solid #cecece; border-left: 1px solid #cecece; padding: 30px 20px 20px 20px`">
                  ...
                  <div v-if="isLoad && all" style="display: flex;width: 100%;align-items: flex-start;justify-content: space-between;text-align: left;">
                    ...
                    <div style="display: flex;width: 100%;align-items: flex-start;justify-content: space-between;text-align: left;flex-direction: column">
                            ...
                            <div style="height: 100%; width: 100%;margin-top: 15px; display:flex; align-items: flex-start; flex-wrap: wrap">
                                <div style="margin-bottom: 15px;width: 370px;border-right: 1px solid #ececec; height: 212px; display: flex; flex-direction: column; justify-content: flex-start; border-bottom: 1px solid rgb(236, 236, 236)">
                                    ...
                                    <div class="filterDiv" style="display: flex; align-items: flex-start; flex-direction: column; margin-top: 5px; overflow-y: scroll; height: 100%; padding-left: 10px">
                                        <div v-for="(solution, i) in getCompanyData.solutions.filter(({active}) => active)" :key="i" style="margin: 4px 0 4px 0;display: flex; align-items: center" @click="topicArray.some(mySolution => mySolution._id == solution._id) ? removeItemFromTopicArray(solution._id) : addItemToTopicArray(solution)">
                                            ...
                                            <div style="margin-left: 12px; margin-right: 0px; width: 98px; height: 18px; background-color: #eff3f5">
                                              <div :style="`height: 18px; width: ${filterBar(solution)}%; background: #3367d6;`"></div>
                                            </div>
                                ...
                                <div v-for="(solution, i) in getCompanyData.solutions.filter(({ active }) => active).slice(0, pageNumberSolution)" :key="i" style="margin-bottom: 15px;width: 370px;height: 212px;display: flex;flex-direction: column;justify-content: flex-start; border-right: 1px solid rgb(236, 236, 236);border-bottom: 1px solid rgb(236, 236, 236);marign-top: 0px">
                                  <div style="padding: 10px 10px 4px 24px; border-bottom: 2px solid rgb(245, 245, 247)">
                                    <span style="font-family: 'Roboto-bold'; color: #333333; font-size: 14px">Estadisticas</span>
                                  </div>
                        ...
                        <!-- <div style="margin-bottom: 24px">
                          <span style="font-size: 14px; color: #333333">Mostrando <span style="font-weight: bold">{{getAllValidations()}}</span> estadisticas de {{getCompanyData.name}}</span>
                        </div> -->
                        <div style="display: flex; align-items: flex-start;">
                            ...
                            <div style="margin-left: 30px;display: flex;border: 1px solid #ccc;flex-direction: column;width: 345px;background: white;border-radius: 2px;">
                                <div style="margin-top: 0px; text-align: left; width: 100%; background: #fafafa; padding: 0px; border-bottom: 1px solid #ececec; display: flex; flex-direction: column">
                                  <div style="padding: 10px 20px 32px; display: flex; align-items: center; justify-content: space-between">
                                    <span style="color: #111111; font-size: 18px; font-family: 'Roboto-regular';">Filtrar Validaciones</span>
                                    <div>
                                      <v-icon @click="filters = 0; filterSelected = 'company'" center style="cursor: pointer; color: #999999; font-size: 14px; margin-left: 10px">fas fa-chevron-left</v-icon>
                                      <v-icon @click="filters = 1; filterSelected = 'size'" center style="cursor: pointer; color: #999999; font-size: 14px; margin-left: 10px">fas fa-chevron-right</v-icon>
                                    </div>
                                  </div>
                                  <div v-if="filters == 0" style="display: flex; justify-content: space-between; padding: 0 5px">
                                    <div @click="filterSelected = 'company'" :style="`padding: 0px 2px 10px; border-bottom: ${filterSelected == 'company' ? '2px solid #4285f4' : ''}; cursor: pointer; color: ${filterSelected == 'company' ? '#4285f4' : '#727273'}; font-size: 13px;`">Nombre Empresa</div>
                                    <div @click="filterSelected = 'title'" :style="`padding: 0px 2px 10px; border-bottom: ${filterSelected == 'title' ? '2px solid #4285f4' : ''}; cursor: pointer; color: ${filterSelected == 'title' ? '#4285f4' : '#727273'}; font-size: 13px;`">Titulo Puesto</div>
                                    <div @click="filterSelected = 'industry'" :style="`padding: 0px 2px 10px; border-bottom: ${filterSelected == 'industry' ? '2px solid #4285f4' : ''}; cursor: pointer; color: ${filterSelected == 'industry' ? '#4285f4' : '#727273'}; font-size: 13px;`">Industrias</div>
                                  </div>
                                  <div v-if="filters == 1" style="display: flex; justify-content: space-between; padding: 0 5px">
                                    <div @click="filterSelected = 'size'" :style="`padding: 0px 2px 10px; border-bottom: ${filterSelected == 'size' ? '2px solid #4285f4' : ''}; cursor: pointer; color: ${filterSelected == 'size' ? '#4285f4' : '#727273'}; font-size: 13px;`">Tamaño Empresa</div>
                                    <div @click="filterSelected = 'position'" :style="`padding: 0px 2px 10px; border-bottom: ${filterSelected == 'position' ? '2px solid #4285f4' : ''}; cursor: pointer; color: ${filterSelected == 'position' ? '#4285f4' : '#727273'}; font-size: 13px;`">Tipo de puesto</div>
                                    <div @click="filterSelected = 'department'" :style="`padding: 0px 2px 10px; border-bottom: ${filterSelected == 'department' ? '2px solid #4285f4' : ''}; cursor: pointer; color: ${filterSelected == 'department' ? '#4285f4' : '#727273'}; font-size: 13px;`">Departamento</div>
                                  </div>
                                </div>
                                
                                <div style="padding: 20px;font-family: Roboto-regular;display: flex;height: 100%;flex-wrap: wrap;">
                                    <!-- <span v-if="getFilteredItems.length == 0" style="padding: 15px"><span style="font-size: 14px; color: #333333">No hay validaciones</span></span> -->
                                     <!-- @click="filterByIndustry = false; filterByIndustry = true; filterValidationsKey.some(value => value.toLowerCase().trim() == item.toLowerCase().trim()) ? removeValidationsByKey(itemitem) : filterValidations(item, filterSelectedoLowerCase.trim)" -->
                                    <div 
                                        v-for="(item, index) in Array.isArray(getFilteredItems) ? getFilteredItems : getFilteredItems.items" 
                                        :key="index"
                                        @click="filterAction(item)"
                                        :style="`${filterValidationsKey.some(value => value.toLowerCase().trim() == (Array.isArray(getFilteredItems) ? item.toLowerCase().trim() : item.type.toLowerCase().trim())) ? 'border-bottom: 3px solid blue; border-top: 1px solid #ccc; border-left: 1px solid #ccc; border-right: 1px solid #ccc' : 'border: 1px solid #ccc'}; margin: 3px;width: fit-content;height: 55px;background: white;display: flex;border-radius: 3px;align-items: center;padding: 20px;cursor: pointer;`"
                                    >
                                        <!-- <img v-if="filterValidationsKey.filter(item => item == item).length == 0" src="/static/images/newCheck.png" style="margin-right: 8px; cursor: pointer; width: 16px" alt="icon header stadistics"> -->
                                        <!-- <img v-if="filterValidationsKey.filter(item => item == item).length > 0" src="/static/images/checkbox_selected.png" style="border: 1px solid #ccc; border-radius: 2px; margin-right: 8px; cursor: pointer; width: 16px" alt="icon header stadistics"> -->
                                        <span v-if="item && Array.isArray(getFilteredItems)" :style="`font-family: 'Roboto-regular'; font-size: 13px; color: #333333;`">{{transformString(item)}}</span>
                                        <span v-if="item && !Array.isArray(getFilteredItems)" :style="`font-family: 'Roboto-regular'; font-size: 13px; color: #333333;`">{{transformString(item.desc)}}</span>
                                    </div>
                                    <!-- <li @click="allIndustries = true" v-if="!allIndustries && uniqueIndustries.length > 0"><span :style="`cursor: pointer; font-family: 'Roboto-regular'; font-size: 14px; `"><v-icon  style="color: #696969; font-size: 16px;">keyboard_arrow_down</v-icon>Ver mas</span></li>
                                    <li @click="allIndustries = false" v-if="allIndustries && uniqueIndustries.length > 0"><span :style="`cursor: pointer; font-family: 'Roboto-regular'; font-size: 14px; color: #333333; margin-left: 18px`"><v-icon  style="color: #696969; font-size: 16px;">keyboard_arrow_up</v-icon>Ver menos</span></li> -->
                                </div>
                            </div>
            ...
            <div v-if="!showService" :style="`width: ${!all ? '1222px' : '100%'};border: ${all ? '' : '1px solid #cecece'};background: white;margin-top: 30px;border-top: 2px solid #cecece7a;`">
              <SurveyContact
                :companyName="getCompanyData.name"
                :companyLogo="getCompanyData.logo"
                :solutions="getCategoriesAndLogo"
...
export default {
    name: 'home',
    components: {
        ContactComponent, Tabs, SurveyContact, MobileSurveyCard,SurveyContactMobile, PreviewBox, PreviewOptionsBox,
    ...
    methods: {
      ...
      filterAction(item) {
        ...
        let existsItemInValidationsArray = this.filterValidationsKey.some(myItem => item == (Array.isArray(this.getFilteredItems) ? myItem : myItem.type));
        ...
        if(existsItemInValidationsArray) {
          ...
          if (Array.isArray(this.getFilteredItems)) {
            this.filterValidations(item, this.filterSelected) 
          } else {
            this.filterValidations(item.type, this.filterSelected)
          }
        }
        // if(this.filterValidationsKey.some(myItem => item == (Array.isArray(this.getFilteredItems) ? myItem : myItem.type))) {
        // } else {
        //   // if (Array.isArray(this.getFilteredItems)) {
        //   //   this.filterValidations(item, this.filterSelected) 
        //   // } else {
      ...
      removeValidationsByKey(item) {
        ...
        this.filterValidationsKey = this.filterValidationsKey.filter(value => value !== (Array.isArray(this.getFilteredItems) ? item : item.type));
